...
centity_createString(const char *string)
{
    ...
    CeString *self = malloc(sizeof(CeString) + len - 1);
    self->type = CE_STRING;
    memcpy(&(self->content), string, len + 1);
    return (Centity *)self;
}
...
{
    unsigned int hashmask = (1 << hashbits) - 1;
    CeDict *self = malloc(sizeof(CeList) + hashmask * sizeof(Centity *));
    self->type = CE_DICT;
    self->hashmask = hashmask;
    memset(&(self->memb), 0, (hashmask + 1) * sizeof(CeDictEntry *));
    return (Centity *)self;
}
